import sys
import os
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import pandas as pd
import numpy as np
from datetime import datetime, timedelta
from typing import Dict, List, Optional
import logging
import json

from core.data_collector.price_data import PriceDataCollector
from core.ai_analyzer.technical_analyzer import TechnicalAnalyzer
from core.ai_analyzer.signal_generator import SignalGenerator
from config.settings import (
    FOREX_PAIRS, ACCOUNT_BALANCE, RISK_PER_TRADE,
    ATR_SL_MULTIPLIER, ATR_TP1_MULTIPLIER, ATR_TP2_MULTIPLIER,
    MIN_RISK_REWARD, MIN_SIGNAL_SCORE
)

logger = logging.getLogger(__name__)


class BacktestEngine:

    SPREAD_PIPS = {
        "EUR/USD": 1.5, "GBP/USD": 2.0, "USD/JPY": 1.5,
        "AUD/USD": 2.0, "USD/CAD": 2.5, "EUR/GBP": 2.0,
    }
    SLIPPAGE_PIPS = 0.5

    def __init__(self, initial_balance: float = None):
        self.initial_balance = initial_balance or ACCOUNT_BALANCE
        self.price_collector = PriceDataCollector()
        self.tech_analyzer = TechnicalAnalyzer()
        self.signal_gen = SignalGenerator()
        self.trades: List[Dict] = []
        self.equity_curve: List[Dict] = []

    def _pip_value(self, symbol: str) -> float:
        return 0.0001 if "JPY" not in symbol else 0.01

    def _apply_spread(self, price: float, direction: str, symbol: str) -> float:
        spread = self.SPREAD_PIPS.get(symbol, 2.0) * self._pip_value(symbol)
        slippage = self.SLIPPAGE_PIPS * self._pip_value(symbol)
        return price + spread + slippage if direction == "LONG" else price - spread - slippage

    def _backtest_risk_check(self, signal: Dict, balance: float) -> Optional[Dict]:
        """Risk check per backtesting — bypassa weekend filter"""
        entry_price = signal.get("entry_price", 0)
        direction = signal.get("direction")
        atr = signal.get("atr", 0.001)
        raw_score = signal.get("raw_score", 0)

        if raw_score < MIN_SIGNAL_SCORE:
            return None
        if entry_price <= 0:
            return None
        if atr <= 0:
            atr = entry_price * 0.001

        if direction == "LONG":
            stop_loss = entry_price - (atr * ATR_SL_MULTIPLIER)
            take_profit_1 = entry_price + (atr * ATR_TP1_MULTIPLIER)
            take_profit_2 = entry_price + (atr * ATR_TP2_MULTIPLIER)
        else:
            stop_loss = entry_price + (atr * ATR_SL_MULTIPLIER)
            take_profit_1 = entry_price - (atr * ATR_TP1_MULTIPLIER)
            take_profit_2 = entry_price - (atr * ATR_TP2_MULTIPLIER)

        sl_distance = abs(entry_price - stop_loss)
        tp1_distance = abs(entry_price - take_profit_1)
        risk_reward = tp1_distance / sl_distance if sl_distance > 0 else 0

        if risk_reward < MIN_RISK_REWARD:
            return None

        risk_amount = balance * RISK_PER_TRADE

        return {
            **signal,
            "stop_loss": round(stop_loss, 5),
            "take_profit_1": round(take_profit_1, 5),
            "take_profit_2": round(take_profit_2, 5),
            "risk_amount": round(risk_amount, 2),
            "risk_reward": round(risk_reward, 2),
            "approved": True,
        }

    def run_backtest(self, symbol: str, df: pd.DataFrame, lookback: int = 60) -> List[Dict]:
        trades = []
        balance = self.initial_balance
        open_trade = None

        logger.info(f"📊 Backtest {symbol}: {len(df)} candele")

        for i in range(lookback, len(df)):
            df_slice = df.iloc[:i+1]
            current_bar = df.iloc[i]
            current_price = float(current_bar["close"])
            current_high = float(current_bar["high"])
            current_low = float(current_bar["low"])

            # Gestisci trade aperto
            if open_trade:
                hit_sl = False
                hit_tp1 = False
                hit_tp2 = False

                if open_trade["direction"] == "LONG":
                    if current_low <= open_trade["stop_loss"]:
                        hit_sl = True
                    elif current_high >= open_trade["take_profit_2"]:
                        hit_tp2 = True
                    elif current_high >= open_trade["take_profit_1"] and not open_trade.get("tp1_hit"):
                        hit_tp1 = True
                else:
                    if current_high >= open_trade["stop_loss"]:
                        hit_sl = True
                    elif current_low <= open_trade["take_profit_2"]:
                        hit_tp2 = True
                    elif current_low <= open_trade["take_profit_1"] and not open_trade.get("tp1_hit"):
                        hit_tp1 = True

                if hit_sl:
                    pnl = -open_trade["risk_amount"]
                    balance += pnl
                    open_trade.update({"exit_price": open_trade["stop_loss"], "exit_reason": "sl",
                                       "pnl": round(pnl, 2), "outcome": "loss", "closed_at": str(df.index[i])})
                    trades.append(open_trade)
                    open_trade = None

                elif hit_tp2:
                    pnl = open_trade["risk_amount"] * open_trade["risk_reward"]
                    balance += pnl
                    open_trade.update({"exit_price": open_trade["take_profit_2"], "exit_reason": "tp2",
                                       "pnl": round(pnl, 2), "outcome": "win", "closed_at": str(df.index[i])})
                    trades.append(open_trade)
                    open_trade = None

                elif hit_tp1:
                    pnl = open_trade["risk_amount"] * open_trade["risk_reward"] * 0.5
                    balance += pnl
                    open_trade["tp1_hit"] = True
                    open_trade["stop_loss"] = open_trade["entry_price"]  # Breakeven

                elif i - open_trade.get("open_bar", i) >= 96:
                    pnl = (current_price - open_trade["entry_price"]) * (1 if open_trade["direction"] == "LONG" else -1)
                    pnl_norm = pnl / max(abs(open_trade["entry_price"] - open_trade["stop_loss"]), 0.0001) * open_trade["risk_amount"]
                    balance += pnl_norm
                    open_trade.update({"exit_price": current_price, "exit_reason": "expired",
                                       "pnl": round(pnl_norm, 2), "outcome": "win" if pnl_norm > 0 else "loss",
                                       "closed_at": str(df.index[i])})
                    trades.append(open_trade)
                    open_trade = None

                self.equity_curve.append({"timestamp": str(df.index[i]), "balance": round(balance, 2)})
                continue

            # Genera segnale
            technical = self.tech_analyzer.get_technical_score(df_slice)
            technical["patterns"] = self.tech_analyzer.detect_patterns(df_slice)
            mock_sentiment = {"combined_score": technical["score"] * 0.3}

            signal = self.signal_gen.check_all_strategies(
                symbol=symbol, technical=technical,
                sentiment=mock_sentiment, current_price=current_price,
            )

            if signal:
                entry_with_spread = self._apply_spread(current_price, signal["direction"], symbol)
                signal["entry_price"] = entry_with_spread
                approved = self._backtest_risk_check(signal, balance)
                if approved:
                    open_trade = {**approved, "open_bar": i, "opened_at": str(df.index[i]), "tp1_hit": False}

            self.equity_curve.append({"timestamp": str(df.index[i]), "balance": round(balance, 2)})

        if open_trade:
            open_trade.update({"exit_reason": "end_of_data", "pnl": 0, "outcome": "open"})
            trades.append(open_trade)

        return trades

    def calculate_metrics(self, trades: List[Dict], symbol: str = "") -> Dict:
        closed = [t for t in trades if t.get("outcome") in ("win", "loss")]
        if not closed:
            return {"symbol": symbol, "error": "Nessun trade chiuso", "total_trades": 0}

        wins = [t for t in closed if t["outcome"] == "win"]
        losses = [t for t in closed if t["outcome"] == "loss"]

        win_rate = len(wins) / len(closed) * 100
        avg_win = np.mean([t["pnl"] for t in wins]) if wins else 0
        avg_loss = abs(np.mean([t["pnl"] for t in losses])) if losses else 0
        total_wins_pnl = sum(t["pnl"] for t in wins)
        total_losses_pnl = abs(sum(t["pnl"] for t in losses))
        profit_factor = (total_wins_pnl / total_losses_pnl) if total_losses_pnl > 0 else 999
        total_pnl = sum(t["pnl"] for t in closed)
        total_pnl_pct = total_pnl / self.initial_balance * 100

        cumulative = np.cumsum([t["pnl"] for t in closed])
        peak = np.maximum.accumulate(cumulative)
        max_dd = float(np.max(peak - cumulative)) / self.initial_balance * 100

        returns = [t["pnl"] / self.initial_balance for t in closed]
        sharpe = (np.mean(returns) / np.std(returns) * np.sqrt(252)) if np.std(returns) > 0 else 0

        consec, max_consec = 0, 0
        for t in closed:
            consec = consec + 1 if t["outcome"] == "loss" else 0
            max_consec = max(max_consec, consec)

        return {
            "symbol": symbol,
            "total_trades": len(closed),
            "wins": len(wins),
            "losses": len(losses),
            "win_rate": round(win_rate, 2),
            "avg_win": round(avg_win, 2),
            "avg_loss": round(avg_loss, 2),
            "profit_factor": round(profit_factor, 2),
            "total_pnl": round(total_pnl, 2),
            "total_pnl_pct": round(total_pnl_pct, 2),
            "max_drawdown_pct": round(max_dd, 2),
            "sharpe_ratio": round(sharpe, 2),
            "max_consecutive_losses": max_consec,
            "avg_rr": round(avg_win / avg_loss, 2) if avg_loss > 0 else 0,
        }

    def run_full_backtest(self, days: int = 365) -> Dict:
        all_results = {}
        all_trades = []
        logger.info(f"🚀 Backtest {days} giorni su {len(FOREX_PAIRS)} coppie")

        for symbol in FOREX_PAIRS:
            logger.info(f"  Analisi {symbol}...")
            df = self.price_collector.get_ohlcv_yahoo(symbol, "1h", days=days)
            if df is None or len(df) < 100:
                continue
            self.equity_curve = []
            trades = self.run_backtest(symbol, df)
            metrics = self.calculate_metrics(trades, symbol)
            all_results[symbol] = metrics
            all_trades.extend(trades)
            logger.info(f"  {symbol}: {metrics.get('total_trades', 0)} trade, WR: {metrics.get('win_rate', 0):.1f}%, PnL: {metrics.get('total_pnl_pct', 0):+.1f}%")

        if all_trades:
            all_results["AGGREGATE"] = self.calculate_metrics(
                [t for t in all_trades if t.get("outcome") in ("win", "loss")], "ALL_PAIRS"
            )
        return all_results


def print_report(results: Dict):
    print("\n" + "="*65)
    print("  PALANTIR — BACKTESTING REPORT")
    print("="*65)
    for symbol, metrics in results.items():
        if "error" in metrics and metrics.get("total_trades", 0) == 0:
            continue
        print(f"\n  {'─'*60}")
        print(f"  {symbol}")
        print(f"  {'─'*60}")
        print(f"  Trade totali:        {metrics.get('total_trades', 0)}")
        print(f"  Win Rate:            {metrics.get('win_rate', 0):.1f}%  {'✅' if metrics.get('win_rate', 0) >= 55 else '❌'}")
        print(f"  Profit Factor:       {metrics.get('profit_factor', 0):.2f}")
        print(f"  Total P&L:           {metrics.get('total_pnl_pct', 0):+.1f}%")
        print(f"  Max Drawdown:        {metrics.get('max_drawdown_pct', 0):.1f}%  {'✅' if metrics.get('max_drawdown_pct', 0) <= 8 else '❌'}")
        print(f"  Sharpe Ratio:        {metrics.get('sharpe_ratio', 0):.2f}  {'✅' if metrics.get('sharpe_ratio', 0) >= 1.2 else '❌'}")
        print(f"  Avg R:R:             1:{metrics.get('avg_rr', 0):.1f}  {'✅' if metrics.get('avg_rr', 0) >= 1.8 else '❌'}")
        print(f"  Max Consec. Losses:  {metrics.get('max_consecutive_losses', 0)}")
    print("\n" + "="*65)


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] — %(message)s")
    engine = BacktestEngine(initial_balance=10000)
    print("🚀 Avvio backtesting su 12 mesi...")
    print("   Attendere 5-10 minuti...\n")
    results = engine.run_full_backtest(days=365)
    with open("backtest_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print_report(results)
    print("\n✅ Risultati salvati in backtest_results.json")
